...
wire	[15:0] lane_pc 			[7:0] ; // pc for each SIMD lane
...
wire	[3:0] lane_branch_depth	[7:0] ;
wire		   lane_predicate		[7:0];
...
generate 
	for (i=0; i<8; i=i+1) begin : thread_freeze
		assign lane_freeze[i] = (lane_freeze_mm[i] | 
				(((lane_block_idx[i] == thread_block_idx_i[0]) & PM_freeze_i[0]) | 
				 ((lane_block_idx[i] == thread_block_idx_i[1]) & PM_freeze_i[1]) |
				 ((lane_block_idx[i] == thread_block_idx_i[2]) & PM_freeze_i[2]) | 
				 ((lane_block_idx[i] == thread_block_idx_i[3]) & PM_freeze_i[3]) |
				 ((lane_block_idx[i] == thread_block_idx_i[4]) & PM_freeze_i[4]) | 
				 ((lane_block_idx[i] == thread_block_idx_i[5]) & PM_freeze_i[5]) |
				 ((lane_block_idx[i] == thread_block_idx_i[6]) & PM_freeze_i[6]) | 
				 ((lane_block_idx[i] == thread_block_idx_i[7]) & PM_freeze_i[7])));
	end
	for (i=0; i<8; i=i+1) begin : block_predicate
		assign predicate_o[i] =  
				((~thread_block_status_o[i] | ((thread_block_idx_i[i] == lane_block_idx[0]) & lane_predicate[0])) | 
				 (~thread_block_status_o[i] | ((thread_block_idx_i[i] == lane_block_idx[1]) & lane_predicate[1])) |
				 (~thread_block_status_o[i] | ((thread_block_idx_i[i] == lane_block_idx[2]) & lane_predicate[2])) | 
				 (~thread_block_status_o[i] | ((thread_block_idx_i[i] == lane_block_idx[3]) & lane_predicate[3])) |
				 (~thread_block_status_o[i] | ((thread_block_idx_i[i] == lane_block_idx[4]) & lane_predicate[4])) | 
				 (~thread_block_status_o[i] | ((thread_block_idx_i[i] == lane_block_idx[5]) & lane_predicate[5])) |
				 (~thread_block_status_o[i] | ((thread_block_idx_i[i] == lane_block_idx[6]) & lane_predicate[6])) | 
				 (~thread_block_status_o[i] | ((thread_block_idx_i[i] == lane_block_idx[7]) & lane_predicate[7])));
	end
...
.lane_branch_depth_o	(lane_branch_depth)
);
...
.PC_i							(lane_pc[0]),
.branch_depth_i				(lane_branch_depth[0]),
...
.PC_i							(lane_pc[1]),
.branch_depth_i				(lane_branch_depth[1]),	
...
.PC_i							(lane_pc[2]),
.branch_depth_i				(lane_branch_depth[2]),	
...
.PC_i							(lane_pc[3]),
.branch_depth_i				(lane_branch_depth[3]),	
...
.PC_i							(lane_pc[4]),
.branch_depth_i				(lane_branch_depth[4]),	
...
.PC_i							(lane_pc[5]),
.branch_depth_i				(lane_branch_depth[5]),	
...
.PC_i							(lane_pc[6]),
.branch_depth_i				(lane_branch_depth[6]),	
...
.PC_i							(lane_pc[7]),
.branch_depth_i				(lane_branch_depth[7]),	
